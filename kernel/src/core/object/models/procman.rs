        ...
        match invocation {
            Invocation::ProcessManager(
                ProcManOp::Spawn { 
                    ...
                    extra_handles_ptr, extra_handles_len,
                    args_buffer_ptr, args_buffer_len,
                }
            ) => {
                ...
                if extra_handles_len > 0 {
                    ...
                    if !success { return Err(InvocationError::InvalidPointer) };
                    ...
                    for grant in parent_grants {
                        ...
                        if let Ok(obj) = parent_proc.proc_handles.read().resolve(grant.id, grant.rights) {
                            ...
                            child_extra_handles.push(
                                HandleGrant { id: chd, rights: grant.rights, tag: grant.tag }
                            );
                        } else {
                ...
                if args_buffer_len > 0 {
                    ...
                    let success = safe_copy_from(
                        args_buffer.as_mut_ptr() as *mut u8,
                        args_buffer_ptr as *const u8,
                        args_buffer_len
                    );
                    if !success { return Err(InvocationError::InvalidPointer); }
                ...
                // inject the payload
                let (pkg_vaddr, safe_stack_top) = ProcessEnvironment::inject(
                    &stack_vmo, stack_addr, stack_size, &child_extra_handles, &args_buffer, argc, initpkg
                )?;
