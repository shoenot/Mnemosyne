...
use crate::kernel::memory::pmm::Allocator;
...
fn get_or_create_next(entry: &mut PageTableEntry, phys_offset: u64, allocator: &'static PhysAlloc) -> Option<*mut PageTable> {
    if !entry.is_present() {
        let new_frame_phys = { allocator.lock().alloc(0)? as u64 };
...
impl PageTable {
    ...
    pub unsafe fn translate(&self, addr: VirtAddress, physical_offset: u64) -> Option<u64> {
        ...
        unsafe {
            ...
            let l1_table = next_table(&(*l2_table).entries[l2 as usize], physical_offset)?;
...
impl Pager {
    pub fn init(allocator: &'static PhysAlloc) -> Option<Self> {
        let pml4_table_frame = { allocator.lock().alloc(0)? as u64 };
