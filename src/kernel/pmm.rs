...
struct Allocator {
    freelist: [usize; ORDER_MAX],
    metadata: &'static mut [u8],
    meta_bit_offset: [usize; ORDER_MAX],
}

impl Allocator {
    fn init(init_allocator: &mut BitmapPMM, max_addr: usize) -> Self {
        ...
        let mut meta_bit_offset = [0; ORDER_MAX];

        meta_bit_offset.iter_mut()
            .enumerate()
            .map(|(order, offset)| {
                *offset = 0 + total_pages / (order * 2);
            });

        let mut allocator = Allocator { freelist: [0; ORDER_MAX], metadata: metadata_slice, meta_bit_offset };
    ...
    fn alloc(&mut self, mut order: usize) -> Option<usize> {
        ...
        let block_addr = { 
            loop {
                ...
                if block.next != 0 {
                    let next_block = unsafe { let blk = block.next as *mut FreeBlock; &mut *blk };
                    next_block.prev = 0;
                }
    ...
    fn free(&mut self, block_addr: usize, order: usize) {
        todo!()
    }

    fn merge_block(&mut self, addr: usize, mut order: usize, target_order: usize) {
        todo!()
    }
}
